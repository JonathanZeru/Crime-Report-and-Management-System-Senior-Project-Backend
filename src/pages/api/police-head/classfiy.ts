  ...
  if (req.method === 'POST') {
    ...
    try {
      if (id || Array.isArray(id)) {
        const caseToBeUpdated = await prisma.case.findUnique(
            {where: { id: Number(id) },
            }
          );
          const severityAnalysis = await analyzeCrimeSeverity(
            caseToBeUpdated.description, caseToBeUpdated.natureOfCrime,
            caseToBeUpdated.crimeDetail);
      ...
      const user = await prisma.case.update(
        ...
        where: { id: Number(id) },
        data: {
            caseLevelOfCrime: caseLevelOfCrime,
            isClassified: true
        }}
      );
